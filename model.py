...
class D(nn.Module):
    def __init__(self):
        ...
        self.norm_c2 = nn.InstanceNorm2d(128, affine=True)
        self.c3 = nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1)
        self.norm_c3 = nn.InstanceNorm2d(256, affine=True)
        self.c4 = nn.Conv2d(256, 256, kernel_size=4, stride=2, padding=1)
        self.norm_c4 = nn.InstanceNorm2d(256, affine=True)
        self.out = nn.Linear(4096, 1)
    
    def forward(self, x):
        x = F.leaky_relu(self.norm_c1(self.c1(x)), 0.2)
        x = F.leaky_relu(self.norm_c2(self.c2(x)), 0.2)
        x = F.leaky_relu(self.norm_c3(self.c3(x)), 0.2)
        x = F.leaky_relu(self.norm_c4(self.c4(x)), 0.2)
        return torch.sigmoid(self.out(x.view(-1, 4096)))
